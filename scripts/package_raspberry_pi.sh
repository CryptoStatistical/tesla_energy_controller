...
apt-get update
apt-get install -y --no-install-recommends bluetooth bluez libcap2-bin openssl python3-venv
...
echo "  3. avviare rete e controller con: sudo systemctl enable --now tesla-energy-controller-network tesla-energy-controller"
INSTALLER
